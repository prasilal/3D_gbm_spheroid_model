...
cfg = Dict(
    ...
    :vaxes => @d(
        ...
        :gbm_p => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.8, :glucose => 0.7), :interphase_duration => 10), 
        :gbm_q => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.5, :glucose => 0.4), :interphase_duration => 10),  
        :gbm_n => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.2, :glucose => 0.1)),
        :gsc_p => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.8, :glucose => 0.7), :interphase_duration => 10),
        :gsc_q => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.7, :glucose => 0.5), :interphase_duration => 10),
        :msc_p => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.7, :glucose => 0.5), :interphase_duration => 10), 
        :msc_anti => @d(:d => 0.0, :D => 0.0, :rd => 0.0, :diffusion_multipliers => @d(:oxygen => 0.5, :glucose => 0.4), :interphase_duration => 10),
    ...
    :rule_graph => @d(
        ...
        :zg => @d(
            ...
            :msc_p => [(:msc_p, :adhesion, 500), (:msc_p, :volume, 1000,20), (:msc_p, :perimeter, 10, 300), (:msc_p, :prod_r, 0.1), (:ecm_gsc, :prod_v, 1.0), (:msc_p, :divide, 0.6)],
            :msc_anti => [(:msc_anti, :adhesion, 600), (:msc_anti, :volume, 1000, 40), (:msc_anti, :perimeter, 10, 500), (:msc_anti, :prod_r, 0.1), (:ecm_barrier, :prod_v, 1.0)],
    ...
    :cells => begin
        ...
        num_cells = NUM_CELLS
        ratios = Dict(:gbm => 0.6, :gsc => 0.2, :msc => 0.2)
        min_dist = 1  # ensure spacing between centers
        local i = 1
        ...
        for (ctype, count) in counts
            for _ in 1:count
                ...
                push!(cells, @d(
                    :state => @d(:type => ctype, :cum_state => cum_state, :cum_state_weight => 1.0, :phase => :interphase, :cell_cycle_timer => rand(1:10)),
                    :init_pos => center,
...
#print_average_volumes_by_type(cfg[:sim])
