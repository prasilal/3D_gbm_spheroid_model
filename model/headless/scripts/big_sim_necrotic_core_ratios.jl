...
function mk_cfg(args)
    ...
    cfg = Dict(
        ...
        :reactions => [
            ...
            @d(:react => [(1, :glucose), (1, :gbm_p)], :prod => [], :k => 0.005, :w => 1.0, :r_absorb => true),
            # GBM Quiescent: reduced uptake
            @d(:react => [(1, :oxygen), (1, :gbm_q)], :prod => [], :k => 0.5, :w => 1.0, :r_absorb => true),
            @d(:react => [(1, :glucose), (1, :gbm_q)], :prod => [], :k => 0.001, :w => 1.0, :r_absorb => true),

            # GSC Proliferating: high oxygen + glucose
            @d(:react => [(1, :oxygen), (1, :gsc_p)], :prod => [], :k => 10.0, :w => 1.0, :r_absorb => true),
            @d(:react => [(1, :glucose), (1, :gsc_p)], :prod => [], :k => 0.005, :w => 1.0, :r_absorb => true),
            # GSC Quiescent: reduced uptake 
            @d(:react => [(1, :oxygen), (1, :gsc_q)], :prod => [], :k => 0.5, :w => 1.0, :r_absorb => true),
            @d(:react => [(1, :glucose), (1, :gsc_q)], :prod => [], :k => 0.001, :w => 1.0, :r_absorb => true),


            # MSC Before:
            @d(:react => [(1, :oxygen), (1, :msc_before)], :prod => [], :k => 1.0, :w => 1.0, :r_absorb => true),
