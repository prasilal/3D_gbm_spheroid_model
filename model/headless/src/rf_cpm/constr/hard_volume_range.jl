...
struct HardVolumeRangeParams
    ...
    lambda_vrange_max
    # indicator if we defined volume constraint
    has_vrange
end
...
function hard_vol_rng_fulfilled(model :: GridModel{T, N, CPMState{Tdc, Trnd, N}, CPMCfg},
    ...
    src_ck = cell_kind(model, src_type)
    if (src_type != BGID && params.has_vrange[src_ck]
        && (cell_volume[src_type] + 1) >
            params.lambda_vrange_max[src_ck])
        return false
    end

    # volume loss of tgt cell
    tgt_ck = cell_kind(model, tgt_type)
    if (tgt_type != BGID && params.has_vrange[tgt_ck]
        && (cell_volume[tgt_type] - 1) <
            params.lambda_vrange_min[tgt_ck])
        return false
    end
