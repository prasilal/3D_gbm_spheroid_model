...
cfg = Dict(
    ...
    :vaxes => @d(
        ...
        :gbm_p => @d(:d => 0.0, :D => 0.0, :rd => 0.01, :diffusion_multipliers => @d(:oxygen => 0.6, :glucose => 0.08)), #0.1
        :gbm_q => @d(:d => 0.0, :D => 0.0, :rd => 0.01, :diffusion_multipliers => @d(:oxygen => 0.4, :glucose => 0.04)), #0.2 
        :gbm_n => @d(:d => 0.0, :D => 0.0, :rd => 0.01, :diffusion_multipliers => @d(:oxygen => 0.2, :glucose => 0.01)), #0.05
    ...
    :rule_graph => @d(
        ...
        :resting_time => 30,
        :zg => @d(
            :gbm_p => [(:gbm_p, :adhesion, 100), (:gbm_p, :volume, 100, 1800), (:gbm_p, :perimeter, 100, 4750), (:gbm_p, :prod_r, 0.1), (:gbm_p, :divide, 5)],
            :gbm_q => [(:gbm_q, :adhesion, 80),  (:gbm_q, :volume, 100, 1800), (:gbm_q, :perimeter, 100, 4750), (:gbm_q, :prod_r, 0.1)],
            :gbm_n => [(:gbm_n, :adhesion, 60),  (:gbm_n, :volume, 100, 1800),  (:gbm_n, :perimeter, 100, 4750), (:gbm_n, :prod_r, 0.1)],
    ...
    :reactions => [
        ...
        @d(:react => [(1, :oxygen), (1, :gbm_n)], :prod => [], :k => 5.0, :w => 1.0, :r_absorb => true),
        @d(:react => [(1, :glucose), (1, :gbm_n)], :prod => [], :k => 5.0, :w => 1.0, :r_absorb => true)
    ],
