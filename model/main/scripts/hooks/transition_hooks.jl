"""
Module: `transition_hooks.jl`

Defines modular cell state transitions for different cell types (GBM, GSC, MSC) based on local chemical environment and internal stress.  
Transitions are dispatched dynamically using `TRANSITION_FN_MAP`, allowing cell-type-specific logic to be modular and extensible.

Key Features:
- Logic-based transitions between proliferative, quiescent, necrotic, and phenotypically altered states
- Local field sensing for oxygen, glucose, lactate, ECM, growth signal, and more
- Stress accumulation and probabilistic transitions
...
"""
try_transition(sim, cell, prob, target_state, vaxmap; stress_bump = 0)

Attempts to transition a `cell` to `target_state` with a given `prob`ability.
- Logs the transition if successful.
- Updates the resting period and optionally reduces stress.
"""
function try_transition(sim, cell, prob, target_state, vaxmap; stress_bump = 0)
    if rand() < prob
        ...
        cell.state[:cum_state] = vaxmap[target_state]              # Set new state
        cell.state[:resting_tracker] = sim.cfg[:rule_graph][:resting_time]  # Enforce rest delay
        if stress_bump > 0
...
# GBM
"""
Transition function for GBM cells.

Handles transitions between the following states:
- `:gbm_p` → Proliferative
- `:gbm_q` → Quiescent
- `:gbm_n` → Necrotic
- `:gsc_p` → Stem-like proliferative (plasticity)

Transition probabilities depend on:
- Local concentrations of oxygen, glucose, lactate, ECM, and growth signal
- Stress accumulation over time
- Environmental score (composite of vaxes)
- ECM cues for dedifferentiation into GSC (via `:ecm_gsc`)
- Growth signal enhancement (modulates reactivation from quiescence)

Notable features:
- Dedifferentiation to GSC under stress + stemness-inducing ECM
- Reversible transitions between proliferative and quiescent states
- Necrosis triggered by persistent hypoxia/starvation
- Receptor values updated to match new state
...
# MSC
"""
Transition function for mesenchymal stromal cells (MSC).

Handles transitions between:
- `:msc_before` → Initial, pre-differentiated MSC
- `:msc_p` → Pro-tumor MSC (promotes GBM growth)
- `:msc_anti` → Anti-tumor MSC (suppresses GBM)

Transition logic includes:
- Hypoxia and acidity promote `:msc_p` behavior
- Low lactate/acidity or low GBM contact favors `:msc_anti`
- GBM contact probability (fraction of GBM neighbors) is a key driver
- ECM or acidosis can trigger reversals
- Initial MSCs (in `:msc_before`) transition with high probability to `:msc_p`

Notable:
- Captures dual role of MSCs in tumor promotion/suppression
- Environmental feedback modulates behavior
- Tracks dynamic stress levels and interaction-dependent plasticity
...
# GSC
"""
Transition function for glioblastoma stem-like cells (GSC).

Handles transitions between:
- `:gsc_p` → Proliferative
- `:gsc_q` → Quiescent
- `:gbm_q` → Differentiated GBM quiescent (loss of stemness)

Transition logic is similar to GBM but with distinct thresholds:
- Hypoxia and starvation induce quiescence
- Stress accumulation promotes quiescence or GBM conversion
- Low ECM stemness signal + high stress leads to GBM transition
- Glucose-triggered reactivation to proliferative state
- Transitions modulated by stemness score (from ECM) and growth signal

Notable:
- Captures stem cell plasticity and context-dependent reversibility
- Sensitive to stemness-supporting ECM conditions
- Reverts to GBM if stemness cues are lost or stress is high
...
"""
modular_transition_dispatcher(sim, cell, in_vaxes)

Top-level dispatcher that selects the appropriate transition function for the cell’s type 
and applies it if the cell is not in its resting period.
"""
...
Implements nutrient uptake using Michaelis-Menten kinetics.
- Computes uptake based on outer voxel field values.
- Applies consumption to `sim.vaxes`.
- Logs per-vax absorption for later plotting.
"""
...
"""
plot_absorb_vaxes(sim, save_dir = nothing)

Generates and optionally saves uptake-over-time plots for each absorbed vax.
"""
...
"""
local_vax_concentration(sim, cell, vaxname)

Returns the average value of `vaxname` field along the cell's boundary voxels.
Returns 0.0 if no boundary pixels are found.
"""
...
"""
log_transition(cell_id, from_state_idx, to_state_idx, sim)

Appends a transition event (cell ID, old state, new state, step) to the global log.
"""
