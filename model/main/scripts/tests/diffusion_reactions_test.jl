...
cfg = Dict(
    ...
    :vaxes => @d(
        ...
        :gbm_p => @d(:d => 0.0, :D => 0.0, :rd => 0.01, :diffusion_multipliers => @d(:oxygen => 0.05, :glucose => 0.0)), #0.1
        :gbm_q => @d(:d => 0.0, :D => 0.0, :rd => 0.01, :diffusion_multipliers => @d(:oxygen => 0.1, :glucose => 0.0)), #0.2 
        :gbm_n => @d(:d => 0.0, :D => 0.0, :rd => 0.01, :diffusion_multipliers => @d(:oxygen => 0.01, :glucose => 0.0)) #0.05
    ),
    ...
    :rule_graph => @d(
        ...
        :resting_time => 5,
        :zg => @d(
            :gbm_p => [(:gbm_p, :adhesion, 100), (:gbm_p, :volume, 100, 150), (:gbm_p, :perimeter, 100, 250), (:gbm_p, :prod_r, 0.01), (:gbm_p, :divide, 5)],
            :gbm_q => [(:gbm_q, :adhesion, 80),  (:gbm_q, :volume, 100, 130), (:gbm_q, :perimeter, 100, 220), (:gbm_q, :prod_r, 0.01)],
            :gbm_n => [(:gbm_n, :adhesion, 60),  (:gbm_n, :volume, 50, 100),  (:gbm_n, :perimeter, 80, 150)],
